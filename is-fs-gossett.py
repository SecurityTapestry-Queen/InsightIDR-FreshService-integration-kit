...
IDR_API_GOSSETT = os.getenv("IDR_API_GOSSETT")
FS_API = os.getenv("FS_API")


def whenWasTheLastTime():
    # print("Obtaining Last Time Ran")
    lasttime = open("lasttime-gossett.txt", "r")
    global lasttimedata
...
def getInsightInvestigations():
    print("Getting Open Investigations for Gossett")
    url = "https://us2.api.insight.rapid7.com/idr/v2/investigations"
    headers = {"X-Api-Key": IDR_API_GOSSETT, "Accept-version": "investigations-preview"}
    params = {
...
def updateLastTime():
    lasttime = open("lasttime-gossett.txt", "w")
...
def getInvestigationComments(id):
    ...
    headers = {"X-Api-Key": IDR_API_GOSSETT, "Accept-version": "comments-preview"}
